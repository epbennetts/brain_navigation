...
numGenes_temp = size(prevBestGenes,1)+1;
...
topAccs_errors = NaN(numNoiseSamples,1);

% Loop through different noise levels
for n = 1:numNoiseSamples
    fprintf('(Printed from general) NOISE LEVEL iteration: %d \n', n);
    fprintf('Noise lev: %d \n', noiseLevelSamples(n));
    [indexOrder, geneNames_ranked, balAcc_ranked, confMatrices_ranked, trees_all_clean, balAcc_errors_ranked] = ...
    ...
    topAccs_errors(n) = balAcc_errors_ranked(1);    
